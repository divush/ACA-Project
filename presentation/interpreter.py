...
#Shield=pygame.mixer.Sound("ShieldsOn.ogg")
def cond(l,value):
	pos=l-1
	c=stack[pos]
	a=c.split()
...
def check(s):
	c=0
	for x in s:
		if(x.isalnum() or x=='{' or x=='}'or x=='$' or x=='(' or x==')' or x==' ' or x=='\n'or x in operators):
			continue
		else:
			 c=1
	if(c):
...
robo1=[main1,supp1,shielder1,True,500,0,3,5]
robo2=[main2,supp2,shielder2,True,500,0,3,5]
...
def fire(a,b,pensize):	
	#Fire.play()
	if(pensize==1):
	  a[1].turtlesize(0.75,0.75,0.2)
	if(pensize==2):
	  a[1].turtlesize(1.2,1.2,0.8)
	...
	if(int(a[1].heading())==0):
		if (abs(a[0].ycor()-b[0].ycor())<=10):
			m=int(b[0].xcor()-10)
			health_dec(b,pensize)
			hit=True
		else:
			m=N
		for i in range(int(a[1].xcor()), m, 20):
			a[1].st()
			a[1].fd(10)
			a[1].ht()
			a[1].fd(10)
		...
		if (abs(a[0].xcor()-b[0].xcor())<=10):
			...
			m=N
		for i in range(int(a[1].ycor()), m, 20):
			a[1].st()
			a[1].fd(10)
			a[1].ht()
			a[1].fd(10)
...
def samepos(a,b):
	if ((a[0].xcor()-b[0].xcor())**2+(a[0].ycor()-b[0].ycor())**2)<400:
	  sys.exit("Match is a Tie")
	...
	if(robo1[life] & robo2[life]):
		...
		if(line2!=inp_len2):
			...
			if(assembly[1][line2]=='moving'):
				...
				fire(robo2,robo1,1)
			elif(assembly[0][line2]=='bombing'):
			  if(robo2[n_bomb]):
			    fire(robo2,robo1,2)
			    robo2[n_bomb]-=1
			elif(assembly[0][line2]=='shielding'):
			  if(robo2[n_shield]):
			    robo2[shield]=50
			    robo2[n_shield]-=1
			    #Shield.play()
		...
		print("Match was a tie")
		break
	print('Health of robo1 is:'+str(robo1[health]))
	print('Health of robo2 is:'+str(robo2[health]))
	if(line1<inp_len1):
		line1+=1
	if(line2<inp_len2):
		line2+=1
